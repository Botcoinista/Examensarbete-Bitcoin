document.addEventListener("DOMContentLoaded", function () {
  ...
  if (modal) {
    ...
    function showModal(imageSrc, title, text, hiddenText, spotifyUrl) {
      document.getElementById("modal-image").src = imageSrc;
      document.getElementById("modal-title").textContent = title;
      document.getElementById("modal-text").textContent = text;
      document.getElementById("modal-hidden-text").textContent = hiddenText;
      document.querySelector('#modal a.no-underline').href = spotifyUrl; // Update Spotify link
      ...
      cards.forEach(card => {
        card.addEventListener("click", function () {
          ...
          if (screenWidth < 500) {
            ...
            const spotifyUrl = card.getAttribute("data-spotify-url"); // Get Spotify URL

            showModal(imageSrc, title, text, hiddenText, spotifyUrl);
          }
        });
      });
    }
